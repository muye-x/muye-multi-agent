...
> 当前阶段：阶段 3 - Generator 与开发者工作流（已完成）
> 更新日期：2026-07-30
...
提供待确认 checksum，避免开发者看到旧 Diff 后确认新 Proposal。
...
Markdown 到 Agent 目录和 Docker/Milvus 本地验证已完成；阶段 4 才引入 Docling/PDF/DOCX 解析能力。PDF
解析及 Compose/Catalog/User grant/MainAgent/citation 全链路仍未完成，不能将阶段 1 视为验收通过。
...
当前进度（2026-07-30）：已完成。正式 Generator 只读取受限的逻辑 Resource、Skill 和 Profile 配置；在
staging 目录完成 descriptor、compile/import smoke 与 provenance 后，使用 no-replace 原子 rename 发布到
`agents/agent-<slug>/`。`validate` 将开发者接管后的源码差异报告为 drift，`diff` 只在内存中重渲染，二者
均拒绝 symlink 与目录边界逃逸。golden、确定性、覆盖保护、输入拒绝、独立生成 Agent contract、只读 diff
和 workspace 不写入均已有回归测试；CI 在运行生成器测试后检查 Git 工作区无变化。

阶段 4 前 `knowledge analyze/approve-*` 只校验并确认版本化逻辑输入的 checksum；`knowledge build/evaluate`
会明确拒绝执行，避免没有 Knowledge Worker/Job 契约时产生半成品。

### 阶段 4：知识 Pipeline 与 Milvus（3 至 4 周）

- PDF/DOCX/MD/TXT Parser、ParsedDocument 和可选 OCR Worker。
- Schema Proposal、人工 checksum 确认、CollectionIndexPlan。
- Dense + BM25/Sparse、Weighted RRF、可选 Rerank、引用和评测。
- Resource Snapshot 与 `muye-data` 原子 reload。
- CLI Job 状态、取消、重试和报告。

退出条件：四类文件通过；扫描 PDF 行为明确；Milvus schema/function/index checksum 验证通过；RAG 门禁建立。

### 阶段 5：Catalog、MainAgent 与权限（2 至 3 周）

- Agent Descriptor 扫描、Catalog/Compose aggregate 和 candidate 校验。
- Control Catalog 投影、health collector、Main ETag reload 和 ACK。
- 请求级授权工具投影、调用前复核、服务身份和 citation 授权。
- Main -> Sub 单向架构测试、circuit breaker、取消和错误投影。
- agent sync/build/deploy/stop/rollback 脚本。

退出条件：零/多 Agent 均可运行；未授权工具不进入模型上下文；Catalog 失败保留旧版本；反向调用被拒绝。

### 阶段 6：Control/Web 与 Compose 发布（2 至 3 周）

- Cool 基座清理、Argon2id、唯一 Admin、无角色普通用户、sessions 和 grants。
- Vue 状态、拓扑、Agent 详情和 User-Agent 映射页面。
- Gateway session introspection、CallerContext、SSE 和公网边界。
- Core Compose、generated Agent Compose、doctor/smoke、资源限制和备份恢复。
